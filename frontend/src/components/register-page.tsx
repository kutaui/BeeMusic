...
import { LOGIN_MUTATION, LOGOUT_MUTATION, REGISTER_MUTATION, VALIDATE_JWT_MUTATION } from '@/graphql/mutations/user-mutation';
...
const formSchema = z.object({
    ...
    username: z.string().min(3, {message: 'Username must be longer than 3 character(s)'}).max(100),
        ...
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 w-[70%] mx-auto  ">
                ...
                <div className="absolute bottom-10 w-[70%]">
                    <h3 className="flex items-center justify-center pb-3">Have an account? <Link href="/login" className="pl-2 font-bold"> Login</Link></h3>
                    <Button variant="formMB" type="submit" className="h-14 ">Register</Button>
                </div>
            </form>
        </Form>
    );
    ...
    return (
        <>
            <div className="relative flex justify-center pb-10">
                <Link href="/" className="font-bold w-[12%] max-w-[60px] h-10 mt-[5%] absolute left-10">
                    <Image width={40} height={40} src="/icons/return.png" alt="Go back icon"
                           className="w-full"/>
                </Link>
